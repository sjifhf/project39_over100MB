...
public class yelevator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(transform.position.y >= startPos.y + height || ontop == -1)
        {
            movingUp = false;
        ...
        if(ontop==-1 && transform.position.y != startPos.y)//玩家離開平台
        {
